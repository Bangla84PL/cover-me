...
export default function Home() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      n8nFormData.append('file', cvFile)
      n8nFormData.append('email', email)
      n8nFormData.append('jobUrl', jobUrl)

      const n8nResponse = await fetch('/api/trigger-n8n', {
        method: 'POST',
        body: n8nFormData
      })
      ...
      if (!n8nResponse.ok) {
        throw new Error(n8nResult.error || 'Failed to trigger n8n workflow')
      }

      // For now, show a success message that n8n is processing
      setGeneratedLetter(`Your cover letter is being processed by our AI workflow.

File ID: ${n8nResult.fileId}
File uploaded: ✅ ${n8nResult.fileName}
n8n workflow: ${n8nResult.webhookTriggered ? '✅ Triggered successfully' : '⚠️ Webhook failed but file uploaded'}

The n8n workflow will:
1. Extract text from your CV using AI
2. Analyze the job posting requirements
3. Generate a personalized cover letter with Claude
4. Create a professional PDF document
5. Save the results back to Supabase

You will receive an email notification when your cover letter is ready for download.

Email: ${email}
Job URL: ${jobUrl}

This is a placeholder message. Your actual cover letter will be generated by the n8n workflow and returned as a PDF.`)
      setProgress(100)
      setStep('result')
    } catch (error) {
